...
function ClassroomInner({ sessionRequestId, currentUser }) {
  ...
  useEffect(() => {
    if (!currentUser || isTutor || joined) return
    setLoading(false)
  ...
  const handleResizeStart = (e, direction) => {
    ...
    const onMove = (e) => {
      ...
      if (direction.includes('left'))  { newW = Math.max(260, startW - dx); newRight = Math.max(0, startRight + dx) }
      if (direction.includes('right')) { newW = Math.max(260, startW + dx) }
      if (direction.includes('top'))   { newH = Math.max(200, startH - dy); newBottom = Math.max(0, startBottom + dy) }
      if (direction.includes('bottom')){ newH = Math.max(200, startH + dy) }
  ...
  return (
    <div
      className={`classroom-container classroom-page-root ${minimized ? 'classroom-minimized' : ''}`}
      style={minimized ? {
        right: pos.right,
        bottom: pos.bottom,
        width: pipSize.width,
    ...
    >
      ...
      <div className="classroom-controls">
        <div className="ctrl-bar">
          {!minimized && (
            <>
              ...
              >
                ...
                <span className="ctrl-label">People ({Object.keys(roster).length})</span>
              </button>
          ...
          <button className="ctrl-btn ctrl-btn--end" onClick={handleEndMeeting}>
